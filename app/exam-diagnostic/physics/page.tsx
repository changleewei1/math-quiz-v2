...
export default function ExamDiagnosticPhysicsPage() {
  ...
  const loadQuestions = async () => {
    ...
    setError('');
    try {
      const res = await fetch('/api/exam-diagnostic?subject=physics');
            ...
            <div className="bg-white rounded-lg shadow-xl p-5 sm:p-6 lg:p-8">
              ...
              <div className="space-y-4">
                <button
                  ...
                  className="w-full px-6 py-3 min-h-[48px] bg-green-500 hover:bg-green-600 text-white rounded-lg shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {loading ? '載入中...' : '開始測驗'}
                </button>
                <Link
